...
const getFileDetailsAndUrl = async (fileId) => {
  if (!gfs) {
    console.warn(`GFS not initialized. Skipping file details fetch for ID: ${fileId}`);
    return null;
  }
  if (!fileId || !mongoose.Types.ObjectId.isValid(fileId)) {
    return null;
  ...
  try {
    ...
    if (files.length > 0) {
      ...
      return {
        ...
        name: file.filename || '',
        url: `http://localhost:5000/uploads/files/${file._id.toString()}`,
      };
    }
  } catch (error) {
...
const populateFormWithFileDetails = async (formDoc) => {
  const formObject = formDoc.toObject ? formDoc.toObject() : { ...formDoc };

  formObject.pdfFiles = [];
  if (formDoc.pdfFileIds && formDoc.pdfFileIds.length > 0) {
    const pdfDetailsPromises = formDoc.pdfFileIds.map(fileId => getFileDetailsAndUrl(fileId));
    const resolvedPdfDetails = (await Promise.all(pdfDetailsPromises)).filter(Boolean);
    formObject.pdfFiles = resolvedPdfDetails;
  }

  formObject.zipFileDetails = null;
  if (formDoc.zipFileId) {
    formObject.zipFileDetails = await getFileDetailsAndUrl(formDoc.zipFileId);
  }

  formObject.groupLeaderSignature = null;
  if (formDoc.groupLeaderSignatureId) {
    formObject.groupLeaderSignature = await getFileDetailsAndUrl(formDoc.groupLeaderSignatureId);
  }

  formObject.guideSignature = null;
  if (formDoc.guideSignatureId) {
    formObject.guideSignature = await getFileDetailsAndUrl(formDoc.guideSignatureId);
  }
